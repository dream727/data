...
int main()
{
	...
	save.setPathTime = "D://data//Time.txt";
	save.setPathThings = "D://data//Things.txt";
	save.SelectedLine = 0;
	...
	ifs.open("D://data//setdata.txt", ios::in);
	if (!ifs.is_open())
	{
		ofs.open("D://data//setdata.txt", ios::out);
	...
	while (true)
	{
		...
		else if (key1 == '5')
		{
			...
			if (key1 == '1')
			{
				cout << "请输入路径（要先把data文件移至指定路径）" << endl;
		...
		else if (key1 == '6')
		{
			...
			if (key1 == '1')
			{
				...
				cin >> inputY;
				while (SelectedIfs >> tempInput1)
				{
					SelectedDatIfs >> tempInput2;
...
void getTime(string& _string)
{
	...
	if (p->tm_mon <= 9)
	{
		_string = to_string(p->tm_year + 1900) + '.' + '0' + to_string(p->tm_mon + 1) + '.' + to_string(p->tm_mday) + '.';
	}
	else
	{
		_string = to_string(p->tm_year + 1900) + '.' + to_string(p->tm_mon + 1) + '.' + to_string(p->tm_mday) + '.';
	}
